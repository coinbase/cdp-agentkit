...
class WowBuyTokenInput(BaseModel):
    ...
    amount_eth_in_wei: str = Field(..., description="Amount of ETH to spend (in wei)", pattern=r"^\d+$")
...
class WowSellTokenInput(BaseModel):
    ...
    contract_address: str = Field(..., description="The WOW token contract address")
    amount_tokens_in_wei: str = Field(..., description="Amount of tokens to sell (in wei)", pattern=r"^\d+$")
